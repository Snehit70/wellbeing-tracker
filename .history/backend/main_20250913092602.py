"""Digital Wellbeing Tracker - Flask Backend (migrated from FastAPI)

This file reproduces the previous FastAPI endpoints and JSON response shapes using
Flask + Flask-Cors. Pydantic models are removed; we construct plain dicts.
"""
...
def debug_overview():
    date_param = request.args.get("date") or datetime.now(timezone.utc)().strftime('%Y-%m-%d')
    issues: List[str] = []

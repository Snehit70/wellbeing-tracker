...
class CategoryManager:
    ...
    def load_categories(self) -> Dict[str, Any]:
        try:
            ...
            abort(500, f"Error loading categories: {e}")
...
def get_categories():
    ...
    if not table_exists('app_categories'):
        return {"categories": []}
    
    # Subquery to get all apps for each category
    app_list = db.session.query(
        m.AppCategory.category,
        func.group_concat(m.AppCategory.app_name).label('apps')
    ).group_by(m.AppCategory.category).subquery()

    # Main query to get category details from the JSON and join with app list
    categories_data = category_manager.load_categories().get("categories", {})
    ...
    categories_dict = {}
    for cat_name, apps_str in results:
        cat_info = categories_data.get(cat_name, {})
        categories_dict[cat_name] = {
            "name": cat_name,
            "apps": apps_str.split(',') if apps_str else [],
            "color": cat_info.get("color", "#9CA3AF"),
...
def remove_app_from_category(category_name: str, app_name: str):
    mapping = m.AppCategory.query.filter_by(app_name=app_name, category=category_name).first()
    if not mapping:

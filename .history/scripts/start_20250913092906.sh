...
# Check for Flask (new backend stack) or gunicorn
...
# Window 3: Backend (Flask via gunicorn)
echo "  🌐 Starting backend API service (Flask/gunicorn)..."
if ! tmux new-window -t $SESSION_NAME -n 'backend'; then
    echo "❌ Failed to create backend window"
    exit 1
fi
tmux send-keys -t $SESSION_NAME:backend 'cd '"$(pwd)"'/backend && echo "=== BACKEND API STARTING (gunicorn) ===" && source ../.venv/bin/activate && gunicorn -w 1 -b 0.0.0.0:8847 main:app' C-m
...
echo "   • (Flask) API Root: http://localhost:8847/"
echo ""
echo "🔧 Tmux commands:"
